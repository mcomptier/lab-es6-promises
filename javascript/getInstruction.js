...
// Bonus 2 - Promise all
const arrayOfBrusselSproutsPromises = [
...
Promise.all(arrayOfBrusselSproutsPromises)
.then((valueOfAllPromises) => {
  valueOfAllPromises.forEach((currentPromise) => {
    document.querySelector("#brusselsSprouts").innerHTML += `<li>${currentPromise}</li>`;
  })
